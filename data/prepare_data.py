...
class BalancedDataPreparator:
    ...
    def download_corpus(self) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Original corpus saved to {corpus_path}")
            self.logger.info(f"Total books collected: {len(corpus_data)}")
            
            # Create deduplicated corpus
            deduplicated_corpus = self.create_deduplicated_corpus(corpus_data)
    ...
    def create_deduplicated_corpus(self, corpus_data: Dict) -> Dict:
        ...
        for book_title, book_data in corpus_data.items():
            ...
            deduplicated_corpus[book_title] = {
            ...
            total_deduplicated += len(cleaned_sentences)
            self.logger.info(f"'{book_title}': {len(original_sentences)} → {len(cleaned_sentences)} sentences")
        
        # Save the deduplicated corpus
        deduplicated_corpus_path = self.data_dir / "corpus_deduplicated.json"
        with open(deduplicated_corpus_path, 'w') as f:
            json.dump(deduplicated_corpus, f, indent=2)
        
        self.logger.info(f"Deduplicated corpus saved to {deduplicated_corpus_path}")
        self.logger.info(f"Total sentences: {total_original} → {total_deduplicated} (removed {total_original - total_deduplicated})")
    ...
    def create_balanced_dataset(self) -> Tuple[DatasetDict, Dict]:
        ...
        elif not deduplicated_corpus_path.exists():
            # Corpus exists but deduplicated version doesn't - create it
        ...
        for book_id, book_data in corpus.items():
            ...
            book_stats[book_id] = {
                'original_sentences': len(book_data['sentences']),
