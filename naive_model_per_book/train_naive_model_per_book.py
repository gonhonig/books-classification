...
class IndividualBookTrainer:
    ...
    def load_data(self):
        ...
        for book_col in self.book_columns:
            ...
            train_labels = [labels[book_idx] for labels in self.dataset['train']['naive_labels']]
            val_labels = [labels[book_idx] for labels in self.dataset['validation']['naive_labels']]
    ...
    def get_split_data(self, split_name: str, book_col: str):
        ...
        labels = [labels_list[book_idx] for labels_list in split_data['labels' if split_name == 'test' else 'naive_labels']]
    ...
    def train_book_model(self, book_col, hidden_dims=None, dropout_rate=None,
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                # Save best model
                torch.save(model.state_dict(), f"naive_model_per_book/{book_name.replace(' ', '_').lower()}_best_model.pth")
            else:
                patience_counter += 1
        ...
        model.load_state_dict(torch.load(f"naive_model_per_book/{book_name.replace(' ', '_').lower()}_best_model.pth"))
    ...
    def train_all_models(self):
        ...
        Path("naive_model_per_book").mkdir(exist_ok=True)
    ...
    def save_results(self):
        ...
        results_path = "naive_model_per_book/results.json"
    ...
    def create_comparison_plot(self):
        ...
        plt.tight_layout()
        plt.savefig('naive_model_per_book/model_comparison.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        logger.info("Comparison plot saved to naive_model_per_book/model_comparison.png")
    ...
    def extract_test_examples(self):
        ...
        for book_col in self.book_columns:
            book_name = self.book_names[book_col]
            model_path = f"naive_model_per_book/{book_name.replace(' ', '_').lower()}_best_model.pth"
        ...
        # Save examples
        examples_path = "naive_model_per_book/naive_per_book_test_examples.json"
        with open(examples_path, 'w') as f:
            json.dump(examples, f, indent=2)
        
        # Create detailed report
        report_path = "naive_model_per_book/naive_per_book_test_examples_report.md"
        with open(report_path, 'w') as f:
            ...
            for i, example in enumerate(examples['multi_label_examples'], 1):
                ...
                for book_col in self.book_columns:
                    ...
                    pred_binary = 1 if pred_prob > 0.5 else 0
                    correct = "✓" if pred_binary == true_label else "✗"
                    f.write(f"| {book_name} | {book_name} | {pred_prob:.3f} ({pred_binary}) | {correct} |\n")
                f.write("\n")
    ...
    def extract_per_book_metrics(self):
        ...
        metrics_path = "naive_model_per_book/naive_per_book_detailed_metrics.json"
        with open(metrics_path, 'w') as f:
            json.dump(per_book_metrics, f, indent=2)
        
        # Create detailed report
        report_path = "naive_model_per_book/naive_per_book_detailed_metrics_report.md"
        with open(report_path, 'w') as f:
