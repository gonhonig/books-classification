...
class ImprovedKNNFeatureExtractor:
    ...
    def __init__(self, config_path: str = "configs/config.yaml"):
        ...
        self.output_dir = Path("data/features_knn_improved")
    ...
    def create_augmented_dataset_improved(self, k_neighbors: int = 5) -> pd.DataFrame:
        ...
        output_path = self.output_dir / "augmented_dataset_improved.csv"
...
def extract_features_improved(config_path: str = "configs/config.yaml", k_neighbors: int = 5):
    """Extract features from fine-tuned model using improved KNN approach."""
    extractor = ImprovedKNNFeatureExtractor(config_path=config_path)
    
    # Create augmented dataset
    df = extractor.create_augmented_dataset_improved(k_neighbors=k_neighbors)
    
    # Create feature summary
    summary = extractor.create_feature_summary_improved(df)
    
    print(f"\n✅ Improved KNN Feature extraction completed!")
...
def main():
    """Main function to extract features using improved KNN approach."""
    parser = argparse.ArgumentParser(description="Extract features using improved KNN approach")
    ...
    try:
        df, summary = extract_features_improved(config_path=args.config, k_neighbors=args.k_neighbors)
        print("Improved KNN Feature extraction completed successfully!")
    except Exception as e:
        logger.error(f"Improved KNN Feature extraction failed: {e}")
        sys.exit(1)
